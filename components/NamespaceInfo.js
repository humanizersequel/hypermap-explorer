...
export default function NamespaceInfo({ slug }) {
  ...
  if (!slug || slug.length === 0) return <div className="text-gray-500 p-4">Enter a valid Hypermap path in the URL (e.g., /nick/hypr).</div>;
  if (isLoadingApi) return <div className="text-gray-500 p-4">Loading entry data for &apos;{fullName}&apos;...</div>;
  if (apiError) return <div className="text-red-500 p-4">Error loading data: {apiError}</div>;
  // Handle case where API call finished but found no data (404 was likely handled by API, but check state too)
  if (!entryData) return <div className="text-gray-500 p-4">Namespace &apos;{fullName}&apos; not found.</div>;
